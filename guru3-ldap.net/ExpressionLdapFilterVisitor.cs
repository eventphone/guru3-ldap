...
{
    public class LdapEventFilterVisitor : LdapFilterVisitor
    {
        public Expression<Func<LdapEvent, bool>> Filter { get; private set; }

        private Stack<Expression> _inner = new Stack<Expression>();
        private readonly ParameterExpression _parameter;

        public LdapEventFilterVisitor()
        {
            _parameter = Expression.Parameter(typeof(LdapEvent), "x");
        }

        public override void Visit(LdapFilter filter)
        {
            base.Visit(filter);
            Filter = Expression.Lambda<Func<LdapEvent, bool>>(_inner.Peek(), _parameter);
        }
        ...
        private MemberExpression GetProperty(string name)
        {
            switch (name.ToLowerInvariant())
            {
                case "ou":
                case "2.5.4.11":
                    return Expression.Property(_parameter, typeof(LdapEvent).GetProperty(nameof(LdapEvent.Name)));
                case "description":
                case "2.5.4.13":
                    return Expression.Property(_parameter, typeof(LdapEvent).GetProperty(nameof(LdapEvent.Description)));
                case "l":
                case "2.5.4.7":
                    return Expression.Property(_parameter, typeof(LdapEvent).GetProperty(nameof(LdapEvent.Location)));
                default:
                    return null;
            }
        }
